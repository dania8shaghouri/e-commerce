...
export const addToCart = (token: string, data: { productId: string; quantity: number }) =>
  axios.post(`${BASE_URL}/cart/items`, data, {
    headers: { Authorization: `Bearer ${token}` },
  });

export const updateCartItem = (token: string, data: { productId: string; quantity: number }) =>
